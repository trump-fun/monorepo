...
contract BettingContract is Ownable {
  ...
  function placeBet(
    ...
    for (uint256 i = 0; i < userBetIds.length; i++) {
      ...
      if (
        userBet.poolId == poolId && userBet.option == optionIndex && userBet.tokenType == tokenType
      ) {
        betId = userBetIds[i];
        break;
      }
  ...
  function claimPayouts(uint256[] memory betIds) public {
    for (uint256 i = 0; i < betIds.length; i++) {
      ...
      uint256[2] storage betTotals = tokenType == TokenType.USDC
        ? pools[poolId].usdcBetTotals
        : pools[poolId].pointsBetTotals;
      ...
      if (bets[betId].option == pools[poolId].winningOption) {
        uint256 winAmount = (bets[betId].amount * betTotals[losingOption]) /
          betTotals[pools[poolId].winningOption] +
          bets[betId].amount;
