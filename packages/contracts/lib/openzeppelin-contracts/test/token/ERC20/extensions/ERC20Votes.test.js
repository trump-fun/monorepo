    ...
    describe(`vote with ${mode}`, function () {
      ...
      describe('set delegation', function () {
        ...
        describe('with signature', function () {
          ...
          it('accept signed delegation', async function () {
            ...
            const tx = await this.token.delegateBySig(
              this.holder,
              nonce,
              ethers.MaxUint256,
              v,
          ...
          it('rejects bad delegatee', async function () {
            ...
            const { args } = await tx
              .wait()
              .then(receipt =>
                receipt.logs.find(event => event.fragment.name == 'DelegateChanged')
              );
          ...
          it('rejects bad nonce', async function () {
            ...
            await expect(
              this.token.delegateBySig(this.holder, nonce + 1n, ethers.MaxUint256, v, r, s)
            )
              .to.be.revertedWithCustomError(this.token, 'InvalidAccountNonce')
      ...
      describe('transfers', function () {
        ...
        afterEach(async function () {
          ...
          expect(await this.token.getPastVotes(this.recipient, timepoint)).to.equal(
            this.recipientVotes
          );
        });
      });
      ...
      describe('Compound test suite', function () {
        ...
        describe('numCheckpoints', function () {
          it('returns the number of checkpoints for a delegate', async function () {
            ...
            expect(await this.token.checkpoints(this.other1, 0n)).to.deep.equal([
              t1.timepoint,
              100n,
            ]);
            expect(await this.token.checkpoints(this.other1, 1n)).to.deep.equal([
              t2.timepoint,
              90n,
            ]);
            expect(await this.token.checkpoints(this.other1, 2n)).to.deep.equal([
              t3.timepoint,
              80n,
            ]);
            expect(await this.token.checkpoints(this.other1, 3n)).to.deep.equal([
              t4.timepoint,
              100n,
            ]);
            await mine();
          ...
          it('does not add more than one checkpoint in a block', async function () {
            ...
            expect(await this.token.checkpoints(this.other1, 0n)).to.be.deep.equal([
              t1.timepoint,
              80n,
            ]);
            ...
            expect(await this.token.numCheckpoints(this.other1)).to.equal(2n);
            expect(await this.token.checkpoints(this.other1, 1n)).to.be.deep.equal([
              t4.timepoint,
              100n,
            ]);
          });
        ...
        describe('getPastVotes', function () {
          ...
          it('generally returns the voting balance at the appropriate checkpoint', async function () {
            ...
            expect(await this.token.getPastVotes(this.other1, t2.timepoint + 1n)).to.equal(
              supply - 10n
            );
            expect(await this.token.getPastVotes(this.other1, t3.timepoint)).to.equal(supply - 20n);
            expect(await this.token.getPastVotes(this.other1, t3.timepoint + 1n)).to.equal(
              supply - 20n
            );
