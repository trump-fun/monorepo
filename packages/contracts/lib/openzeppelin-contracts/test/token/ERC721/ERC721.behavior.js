      ...
      const transferWasSuccessful = () => {
        ...
        it('emits a Transfer event', async function () {
          await expect(this.tx())
            ...
            .withArgs(this.owner, this.to, tokenId);
        });
      ...
      const shouldTransferTokensByUsers = function (fragment, opts = {}) {
        ...
        describe('when sent to the owner', function () {
          ...
          it('emits only a transfer event', async function () {
            await expect(this.tx())
              .to.emit(this.token, 'Transfer')
              .withArgs(this.owner, this.owner, tokenId);
          });
        ...
        describe('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expect(
              this.token
                .connect(this.owner)
                [fragment](this.other, this.other, tokenId, ...(opts.extra ?? []))
            )
        ...
        describe('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expect(
              this.token
                .connect(this.owner)
                [fragment](this.owner, ethers.ZeroAddress, tokenId, ...(opts.extra ?? []))
            )
    ...
    describe('with minted tokens', function () {
      ...
      describe('with burnt token', function () {
        ...
        it('emits a Transfer event', async function () {
          await expect(this.tx)
            .to.emit(this.token, 'Transfer')
            .withArgs(this.owner, ethers.ZeroAddress, firstTokenId);
        });
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns correct token IDs for target', async function () {
          ...
          expect(
            await Promise.all([0n, 1n].map(i => this.token.tokenOfOwnerByIndex(this.other, i)))
          ).to.have.members([firstTokenId, secondTokenId]);
        });
      ...
      for (const tokenId of [firstTokenId, secondTokenId]) {
        it(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function () {
          ...
          expect(await Promise.all([0n, 1n, 2n].map(i => this.token.tokenByIndex(i))))
            .to.have.members(
              [firstTokenId, secondTokenId, newTokenId, anotherNewTokenId].filter(
                x => x !== tokenId
              )
            )
