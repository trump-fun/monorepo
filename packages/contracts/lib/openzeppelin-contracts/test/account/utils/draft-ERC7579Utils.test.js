...
describe('ERC7579Utils', function () {
  ...
  describe('execSingle', function () {
    it('calls the target with value', async function () {
      ...
      await expect(this.utils.$execSingle(data, EXEC_TYPE_DEFAULT)).to.emit(
        this.target,
        'MockFunctionCalled'
      );
    ...
    it('calls the targets with value', async function () {
      ...
      const data = encodeBatch(
        ...
        [
          this.anotherTarget,
          value2,
          this.anotherTarget.interface.encodeFunctionData('mockFunction'),
        ]
      );
  ...
  it('encodes single', async function () {
    ...
    expect(this.utils.$encodeSingle(target, value, data)).to.eventually.equal(
      encodeSingle(target, value, data)
    );
  });
  ...
  it('encodes delegate', async function () {
    ...
    expect(this.utils.$encodeDelegate(target, data)).to.eventually.equal(
      encodeDelegate(target, data)
    );
  });
