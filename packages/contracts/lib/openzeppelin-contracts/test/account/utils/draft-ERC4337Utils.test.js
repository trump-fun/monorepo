...
describe('ERC4337Utils', function () {
  ...
  describe('parseValidationData', function () {
    ...
    it('parse canonical values', async function () {
      expect(this.utils.$parseValidationData(this.SIG_VALIDATION_SUCCESS)).to.eventually.deep.equal(
        [ethers.ZeroAddress, 0n, MAX_UINT48]
      );
  ...
  describe('packValidationData', function () {
    ...
    it('packing reproduced canonical values', async function () {
      ...
      expect(
        this.utils.$packValidationData(ethers.Typed.address(ADDRESS_ONE), 0n, 0n)
  ...
  describe('combineValidationData', function () {
    ...
    it('combines the validation data', async function () {
      ...
      // check symmetry
      expect(
        this.utils.$combineValidationData(validationData1, validationData2)
      ).to.eventually.equal(expected);
      expect(
        this.utils.$combineValidationData(validationData2, validationData1)
      ).to.eventually.equal(expected);
    });
  ...
  describe('getValidationData', function () {
    it('returns the validation data with valid validity range', async function () {
      ...
      expect(this.utils.$getValidationData(validationData)).to.eventually.deep.equal([
        aggregator.address,
        false,
      ]);
    });
    ...
    it('returns address(0) and false for validationData = 0', function () {
      expect(this.utils.$getValidationData(0n)).to.eventually.deep.equal([
        ethers.ZeroAddress,
        false,
      ]);
    });
  ...
  describe('hash', function () {
    it('returns the operation hash with specified entrypoint and chainId', async function () {
      ...
      expect(this.entrypoint.getUserOpHash(userOp.packed)).to.eventually.equal(
  ...
  describe('userOp values', function () {
    describe('intiCode', function () {
      ...
      it('returns factory', async function () {
        ...
        expect(this.utils.$factory(this.emptyUserOp.packed)).to.eventually.equal(
          ethers.ZeroAddress
        );
      });
    ...
    it('returns verificationGasLimit', async function () {
      ...
      expect(this.utils.$verificationGasLimit(userOp.packed)).to.eventually.equal(
    ...
    it('returns maxPriorityFeePerGas', async function () {
      ...
      expect(this.utils.$maxPriorityFeePerGas(userOp.packed)).to.eventually.equal(
        userOp.maxPriorityFee
      );
    });
    ...
    describe('paymasterAndData', function () {
      ...
      it('returns paymaster', async function () {
        expect(this.utils.$paymaster(this.userOp.packed)).to.eventually.equal(
          this.userOp.paymaster
        );
        expect(this.utils.$paymaster(this.emptyUserOp.packed)).to.eventually.equal(
          ethers.ZeroAddress
        );
      });
      ...
      it('returns verificationGasLimit', async function () {
        ...
        expect(
          this.utils.$paymasterVerificationGasLimit(this.emptyUserOp.packed)
      ...
      it('returns postOpGasLimit', async function () {
        ...
        expect(this.utils.$paymasterPostOpGasLimit(this.emptyUserOp.packed)).to.eventually.equal(
          0n
        );
      });

      it('returns data', async function () {
        expect(this.utils.$paymasterData(this.userOp.packed)).to.eventually.equal(
