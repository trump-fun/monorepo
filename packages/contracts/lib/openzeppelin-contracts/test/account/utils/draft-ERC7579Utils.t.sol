...
contract SampleAccount is IAccount, Ownable {
  ...
  IEntryPoint internal constant ENTRY_POINT =
    IEntryPoint(payable(0x0000000071727De22E5E9d8BAf0edAc6f37da032));
...
contract ERC7579UtilsTest is Test {
  ...
  constructor() {
    ...
    vm.etch(
      ...
      vm.readFileBinary("test/bin/SenderCreator070.bytecode")
    );
  ...
  function _printDecodedCalls(bytes memory logData, bool includeTotalValue) internal pure {
    ...
    for (uint256 i = 0; i < calls.length; ++i) {
      console.log(
        string.concat("    calls[", vm.toString(i), "].target = ", vm.toString(calls[i].target))
      );
      console.log(
        string.concat("    calls[", vm.toString(i), "].value = ", vm.toString(calls[i].value))
      );
      console.log(
        string.concat("    calls[", vm.toString(i), "].data = ", vm.toString(calls[i].callData))
      );
