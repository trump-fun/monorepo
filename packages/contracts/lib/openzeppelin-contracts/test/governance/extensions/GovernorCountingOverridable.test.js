    ...
    describe(`using ${Token}`, function () {
      ...
      it('deployment check', async function () {
        ...
        expect(await this.mock.COUNTING_MODE()).to.equal(
          'support=bravo,override&quorum=for,abstain&overridable=true'
        );
      });
      ...
      describe('cast override vote', async function () {
        ...
        it('override after delegate vote', async function () {
          ...
          await expect(
            this.mock
              .connect(this.voter1)
              .castOverrideVote(this.helper.id, VoteType.Against, reason)
          )
        ...
        it('override before and after delegate vote', async function () {
          ...
          await expect(
            this.mock
              .connect(this.voter4)
              .castOverrideVote(this.helper.id, VoteType.Abstain, reason2)
          )
            .to.emit(this.mock, 'OverrideVoteCast')
            .withArgs(
              this.voter4,
