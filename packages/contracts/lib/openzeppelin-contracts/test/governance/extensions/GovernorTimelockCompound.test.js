  ...
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`using ${Token}`, function () {
      ...
      it("doesn't accept ether transfers", async function () {
        await expect(
          this.owner.sendTransaction({ to: this.mock, value: 1n })
        ...
        describe('on queue', function () {
          ...
          it('if proposal contains duplicate calls', async function () {
            const action = {
              ...
              data: this.token.interface.encodeFunctionData('approve', [
      ...
      describe('onlyGovernance', function () {
        describe('relay', function () {
          ...
          it('can be executed through governance', async function () {
            ...
            await expect(txExecute).to.changeTokenBalances(
              this.token,
              [this.mock, this.other],
              [-1n, 1n]
            );
        ...
        describe('updateTimelock', function () {
          beforeEach(async function () {
            this.newTimelock = await ethers.deployContract('CompTimelock', [
              this.mock,
              time.duration.days(7n),
            ]);
          });
        ...
        it('can transfer timelock to new governor', async function () {
          ...
          await expect(this.helper.execute())
            .to.emit(this.timelock, 'NewPendingAdmin')
            .withArgs(newGovernor);
