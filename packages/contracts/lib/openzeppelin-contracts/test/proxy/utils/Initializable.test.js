...
describe('Initializable', function () {
  describe('basic testing without inheritance', function () {
    ...
    describe('after initialize', function () {
      ...
      it('initializer does not run again', async function () {
        await expect(this.mock.initialize()).to.be.revertedWithCustomError(
          this.mock,
  ...
  describe('disabling initialization', function () {
    it('old and new patterns in bad sequence', async function () {
      ...
      await expect(DisableBad1.deploy()).to.be.revertedWithCustomError(
        DisableBad1,
        'InvalidInitialization'
      );

      const DisableBad2 = await ethers.getContractFactory('DisableBad2');
      await expect(DisableBad2.deploy()).to.be.revertedWithCustomError(
        DisableBad2,
        'InvalidInitialization'
      );
    });
