...
describe('UUPSUpgradeable', function () {
  ...
  it('upgrade to upgradeable implementation with call', async function () {
    ...
    await expect(
      this.instance.upgradeToAndCall(
        this.implUpgradeOk,
  ...
  it('calling upgradeTo on the implementation reverts', async function () {
    await expect(
      this.implInitial.upgradeToAndCall(this.implUpgradeOk, '0x')
    ).to.be.revertedWithCustomError(this.implInitial, 'UUPSUnauthorizedCallContext');
  });
  ...
  it('calling upgradeToAndCall from a contract that is not an ERC1967 proxy (with the right implementation) reverts', async function () {
    ...
    await expect(
      instance.upgradeToAndCall(this.implUpgradeUnsafe, '0x')
    ).to.be.revertedWithCustomError(instance, 'UUPSUnauthorizedCallContext');
  });
  ...
  it('upgrade to and unsafe upgradeable implementation', async function () {
    ...
    expect(await getAddressInSlot(this.instance, ImplementationSlot)).to.equal(
      this.implUpgradeUnsafe
    );
  });
