...
describe('ERC165Checker', function () {
  ...
  describe('ERC165 missing return data', function () {
    ...
    it('does not support mock interface via getSupportedInterfaces', async function () {
      expect(await this.mock.$getSupportedInterfaces(this.target, [DUMMY_ID])).to.deep.equal([
        false,
      ]);
    });
  ...
  describe('ERC165 and many interfaces supported', function () {
    ...
    it('supports all interfaceIds via getSupportedInterfaces', async function () {
      expect(
        await this.mock.$getSupportedInterfaces(this.target, supportedInterfaces)
      ).to.deep.equal(supportedInterfaces.map(i => supportedInterfaces.includes(i)));
    });
    ...
    it('supports none of the interfaces queried via getSupportedInterfaces', async function () {
      ...
      expect(
        await this.mock.$getSupportedInterfaces(this.target, interfaceIdsToTest)
      ).to.deep.equal(interfaceIdsToTest.map(i => supportedInterfaces.includes(i)));
    });
    ...
    it('supports each interfaceId via supportsERC165InterfaceUnchecked', async function () {
      for (const interfaceId of supportedInterfaces) {
        expect(await this.mock.$supportsERC165InterfaceUnchecked(this.target, interfaceId)).to.be
          .true;
      }
    });
  });
  ...
  describe('account address does not support ERC165', function () {
    ...
    it('does not support mock interface via getSupportedInterfaces', async function () {
      expect(await this.mock.$getSupportedInterfaces(DUMMY_ACCOUNT, [DUMMY_ID])).to.deep.equal([
        false,
      ]);
    });

    it('does not support mock interface via supportsERC165InterfaceUnchecked', async function () {
      expect(await this.mock.$supportsERC165InterfaceUnchecked(DUMMY_ACCOUNT, DUMMY_ID)).to.be
        .false;
    });
  });
