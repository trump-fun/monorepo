...
describe('Strings', function () {
  ...
  describe('base64', function () {
    for (const { title, input, expected } of [
      {
        title: 'converts to base64 encoded string with double padding',
        input: 'test',
        expected: 'dGVzdA==',
      },
      {
        title: 'converts to base64 encoded string with single padding',
        input: 'test1',
        expected: 'dGVzdDE=',
      },
      {
        title: 'converts to base64 encoded string without padding',
        input: 'test12',
        expected: 'dGVzdDEy',
      },
      { title: 'converts to base64 encoded string (/ case)', input: 'où', expected: 'b/k=' },
      {
        title: 'converts to base64 encoded string (+ case)',
        input: 'zs~1t8',
        expected: 'enN+MXQ4',
      },
  ...
  describe('base64url', function () {
    for (const { title, input, expected } of [
      {
        title: 'converts to base64url encoded string with double padding',
        input: 'test',
        expected: 'dGVzdA',
      },
      {
        title: 'converts to base64url encoded string with single padding',
        input: 'test1',
        expected: 'dGVzdDE',
      },
      {
        title: 'converts to base64url encoded string without padding',
        input: 'test12',
        expected: 'dGVzdDEy',
      },
      { title: 'converts to base64url encoded string (_ case)', input: 'où', expected: 'b_k' },
      {
        title: 'converts to base64url encoded string (- case)',
        input: 'zs~1t8',
        expected: 'enN-MXQ4',
      },
      { title: 'empty bytes', input: '', expected: '' },
      ...
      it(title, async function () {
        const buffer = Buffer.from(input, 'ascii');
        expect(await this.mock.$encodeURL(buffer)).to.equal(
          base64toBase64Url(ethers.encodeBase64(buffer))
        );
