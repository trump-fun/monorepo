...
describe('Strings', function () {
  ...
  describe('toHexString', function () {
    it('converts 0', async function () {
      ...
      expect(await this.mock.$tryParseHexUint(string.replace(/0x/, ''))).to.deep.equal([
        true,
        value,
      ]);
    });
  ...
  describe('toHexString fixed', function () {
    ...
    it('converts MAX_UINT256', async function () {
      expect(
        await this.mock.getFunction('$toHexString(uint256,uint256)')(ethers.MaxUint256, 32n)
      ).to.equal(ethers.toBeHex(ethers.MaxUint256));
    });
  });
    ...
    describe('toHexString', function () {
      for (const addr of addresses) {
        it(`converts ${addr}`, async function () {
          expect(await this.mock.getFunction('$toHexString(address)')(addr)).to.equal(
            addr.toLowerCase()
          );
        });
      }
    ...
    describe('parseAddress', function () {
      for (const addr of addresses) {
        it(`converts ${addr}`, async function () {
          ...
          expect(await this.mock.$tryParseAddress(addr)).to.deep.equal([
            true,
  ...
  describe('Edge cases: invalid parsing', function () {
    it('parseUint overflow', async function () {
      await expect(
        this.mock.$parseUint((ethers.MaxUint256 + 1n).toString(10))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
      await expect(
        this.mock.$tryParseUint((ethers.MaxUint256 + 1n).toString(10))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
    });

    it('parseUint invalid character', async function () {
      await expect(this.mock.$parseUint('0x1')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseUint('1f')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseUint('-10')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseUint('1.0')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseUint('1 000')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
    ...
    it('parseUint invalid range', async function () {
      expect(this.mock.$parseUint('12', 3, 2)).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
    ...
    it('parseInt overflow', async function () {
      await expect(
        this.mock.$parseInt((ethers.MaxUint256 + 1n).toString(10))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
      await expect(
        this.mock.$parseInt((-ethers.MaxUint256 - 1n).toString(10))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
      await expect(
        this.mock.$tryParseInt((ethers.MaxUint256 + 1n).toString(10))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
      await expect(
        this.mock.$tryParseInt((-ethers.MaxUint256 - 1n).toString(10))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
      await expect(
        this.mock.$parseInt((ethers.MaxInt256 + 1n).toString(10))
      ).to.be.revertedWithCustomError(this.mock, 'StringsInvalidChar');
      await expect(
        this.mock.$parseInt((ethers.MinInt256 - 1n).toString(10))
      ).to.be.revertedWithCustomError(this.mock, 'StringsInvalidChar');
      expect(await this.mock.$tryParseInt((ethers.MaxInt256 + 1n).toString(10))).to.deep.equal([
        false,
        0n,
      ]);
      expect(await this.mock.$tryParseInt((ethers.MinInt256 - 1n).toString(10))).to.deep.equal([
        false,
        0n,
      ]);
    });
    ...
    it('parseInt invalid character', async function () {
      await expect(this.mock.$parseInt('0x1')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseInt('1f')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseInt('1.0')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseInt('1 000')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
    ...
    it('parseInt invalid range', async function () {
      expect(this.mock.$parseInt('-12', 3, 2)).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
    ...
    it('parseHexUint overflow', async function () {
      await expect(
        this.mock.$parseHexUint((ethers.MaxUint256 + 1n).toString(16))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
      await expect(
        this.mock.$tryParseHexUint((ethers.MaxUint256 + 1n).toString(16))
      ).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_OVERFLOW);
    });
    ...
    it('parseHexUint invalid character', async function () {
      ...
      await expect(this.mock.$parseHexUint('-1')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseHexUint('-f')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseHexUint('-0xf')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseHexUint('1.0')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
      await expect(this.mock.$parseHexUint('1 000')).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
    ...
    it('parseHexUint invalid begin and end', async function () {
      expect(this.mock.$parseHexUint('0x', 3, 2)).to.be.revertedWithCustomError(
        this.mock,
        'StringsInvalidChar'
      );
