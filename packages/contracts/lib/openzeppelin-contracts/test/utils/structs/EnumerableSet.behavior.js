...
function shouldBehaveLikeSet() {
  async function expectMembersMatch(methods, values) {
    ...
    expect(await Promise.all(values.map((_, index) => methods.at(index)))).to.have.deep.members(
      values
    );
  ...
  describe('add', function () {
    it('adds a value', async function () {
      await expect(this.methods.add(this.valueA))
        .to.emit(this.mock, this.events.addReturn)
        .withArgs(true);
  ...
  describe('at', function () {
    it('reverts when retrieving non-existent elements', async function () {
      await expect(this.methods.at(0)).to.be.revertedWithPanic(
  ...
  describe('remove', function () {
    it('removes added values', async function () {
      ...
      await expect(this.methods.remove(this.valueA))
