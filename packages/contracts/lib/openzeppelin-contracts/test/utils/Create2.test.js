...
describe('Create2', function () {
  ...
  describe('deploy', function () {
    it('deploys a contract without constructor', async function () {
      ...
      expect(this.constructorLessBytecode).to.include(
        (await ethers.provider.getCode(offChainComputed)).slice(2)
      );
    });
      ...
      it('bubbles up panic', async function () {
        await expect(
          this.factory.$deploy(
            ...
            ethers.concat([
              this.mockFactory.bytecode,
              this.mockFactory.interface.encodeDeploy([RevertType.Panic]),
            ])
          )
