...
library ERC7579Utils {
  ...
  function encodeBatch(
    Execution[] memory executionBatch
  ) internal pure returns (bytes memory executionCalldata) {
  ...
  function decodeBatch(
    ...
    unchecked {
      ...
      uint256 arrayLength = uint256(
        bytes32(executionCalldata[arrayLengthOffset:arrayLengthOffset + 32])
      );
      ...
      if (
        arrayLength > type(uint64).max || bufferLength - arrayLengthOffset - 32 < arrayLength * 32
