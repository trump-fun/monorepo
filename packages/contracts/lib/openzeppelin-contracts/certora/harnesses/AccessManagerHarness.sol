...
contract AccessManagerHarness is AccessManager {
  ...
  function getAdminRestrictions_restricted(
    bytes calldata data
  ) external view returns (bool result) {
    (result, , ) = _getAdminRestrictions(data);
  }

  function getAdminRestrictions_roleAdminId(
    bytes calldata data
  ) external view returns (uint64 result) {
    (, result, ) = _getAdminRestrictions(data);
  }

  function getAdminRestrictions_executionDelay(
    bytes calldata data
  ) external view returns (uint32 result) {
    (, , result) = _getAdminRestrictions(data);
  }
  ...
  function hasRole_executionDelay(
    uint64 roleId,
    address account
  ) external view returns (uint32 result) {
    (, result) = hasRole(roleId, account);
  ...
  function getAccess_currentDelay(
    uint64 roleId,
    address account
  ) external view returns (uint32 result) {
    (, result, , ) = getAccess(roleId, account);
  ...
  function getAccess_pendingDelay(
    uint64 roleId,
    address account
  ) external view returns (uint32 result) {
    (, , result, ) = getAccess(roleId, account);
